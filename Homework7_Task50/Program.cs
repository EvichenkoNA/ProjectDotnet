...
int[,] Fill2FibArray(int countRow, int countColumn, int fibLenght)
{
    // Создаем одномерный массив с числами Фибоначчи 
    int[] fibonacciArr = new int[fibLenght];       
...
void Print2DArray(int[,] matr, int rowInd, int columnInd)
{
    ...
    for (int i = 0; i < matr.GetLength(0); i++)
    {
        ...
        {
            // Проверяем i и j на соответствие искомым индексам, подсвечиваем зеленым
...
int row = ReadData("Введите количество строк: ");                       // Вводим размеры матрицы
int column = ReadData("Введите количество столбцов: ");

int rowIndex = ReadData("Введите индекс строки искомого элемента: ");       // Вводим индексы искомого элемента
